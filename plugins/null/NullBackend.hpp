...
class NullBackend final : public IRenderBackend {
  public:
    bool init(void*) override {
        core::util::Logger::info("[Null] init");
        return true;
    }
    gfx::CmdHandle beginFrame() override {
        core::util::Logger::info("[Null] beginFrame");
        return {};
    }
    void endFrame(gfx::CmdHandle) override {
        core::util::Logger::info("[Null] endFrame");
    }
    void clearColor(gfx::CmdHandle, TextureHandle, const float[4]) override {
    }
    void shutdown() override {
        core::util::Logger::info("[Null] shutdown");
    }
    void preShutdown() override {
        core::util::Logger::info("[Null] preShutdown");
    }
    ...
    void cmdDrawIndexed(CmdHandle, uint32_t, uint32_t, uint32_t, int32_t, uint32_t) override {
    }

    void transition(gfx::CmdHandle, gfx::TextureHandle, int, int, int, int, int, int) override {
    }
};

/* exported factory */
inline IRenderBackend* createNullBackend() {
