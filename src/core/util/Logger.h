...
class Logger {
public:
    static void init(std::string_view tag = "ULT_GFX") noexcept { s_tag = tag; }
    ...
    template<typename... Args>
    static void warn(std::string_view fmt, Args&&... args)  noexcept {
...
private:
    template<typename... Args>
    static void log(LogLevel lvl, std::string_view fmt, Args&&... args) noexcept {
        const char* prefix =
            (lvl == LogLevel::Info)  ? "[I]" :
            (lvl == LogLevel::Warn)  ? "[W]" : "[E]";
